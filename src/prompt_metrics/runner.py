...
class RubricEvaluatorAdapter:
 ...
 raise AttributeError(f"Evaluator {self.name} has neither evaluate() nor score()")
...
class ExperimentRunner:
 ...
 raise e
 case_latencies["generator"] = int((time.perf_counter() - gen_start) * 1000)

 # Step 2: Run Evaluators (always evaluate even if empty to capture exact scores/failures)
 evaluator_results = {}
 ...
 case_errors.append({"stage": f"evaluator_{ev.name}", "error": str(ev_err)})
 case_latencies[ev.name] = int((time.perf_counter() - ev_start) * 1000)
